          ...
          <div className="space-y-1.5">
            ...
            {use12h ? (
              <Input
                ...
                min={1}
                max={12}
                value={to12h(s.hour).h12}
                onChange={(e) => {
                  const val = parseInt(e.target.value) || 0;
                  const ampm = to12h(s.hour).ampm;
                  if (val >= 1 && val <= 12) updateSchedule(i, { hour: to24h(val, ampm) });
                }}
                className={`h-8 text-sm ${(() => { const v = to12h(s.hour).h12; return v < 1 || v > 12 ? 'border-red-500 focus-visible:ring-red-500' : ''; })()}`}
              />
            ) : (
              <Input type="number" min={0} max={23} value={s.hour} onChange={(e) => {
                const val = parseInt(e.target.value) || 0;
                if (val >= 0 && val <= 23) updateSchedule(i, { hour: val });
              }} className={`h-8 text-sm ${s.hour < 0 || s.hour > 23 ? 'border-red-500 focus-visible:ring-red-500' : ''}`} />
            )}
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs font-medium text-muted-foreground">Minute</Label>
            <Input type="number" min={0} max={59} value={s.minute} onChange={(e) => {
              const val = parseInt(e.target.value) || 0;
              if (val >= 0 && val <= 59) updateSchedule(i, { minute: val });
            }} className={`h-8 text-sm ${s.minute < 0 || s.minute > 59 ? 'border-red-500 focus-visible:ring-red-500' : ''}`} />
          </div>
          {use12h && (
            <div className="space-y-1.5">
